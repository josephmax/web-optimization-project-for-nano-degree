...
// I found it unnecessary to use DOM selector to get items every time and calculate the scrollTop in the loop.
// so i stored them in variables and modify them in 'updatePositions' handler

function updatePositions(items, scrollTop) {
  frame++;
  ticking = false;
  window.performance.mark("mark_start_frame");
  // var items = document.querySelectorAll('.mover');
  Array.prototype.map.call(items, function(item, index) {
...
/**
 * {array} movers - querySelector result to store the moveing picture objects
 * {number} scrollTop - document.body.scrollTop
 * {boolean} ticking - added ticking to use rAF to help better performance when cpu is poor. if ticking is true, position update will not be triggered.
 * {object} randomContainer - stored randomPizzaContainers' oldwidth and their parent's width.
 */
var movers, scrollTop, ticking = false; 
var randomContainer = {
  width: '',
  parentWidth: ''
};
function pageInit() {
  ...
  var cols = 8;
  var s = 256;
  var num = (screen.height / 100) * 8;
  var movingPizzaContianer = document.querySelector("#movingPizzas1");
  for (var i = 0; i < num; i++) {
    ...
    movingPizzaContianer.appendChild(elem);
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  //I moved DOM listeners here to ensure the handlers not to be triggered before DOM is loaded.
